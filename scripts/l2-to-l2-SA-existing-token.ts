...
function printUsage(): never {
  console.error(`
Usage: npx ts-node scripts/l2-to-l2-SA-existing-token.ts --source <a|b> --dest <a|b>

  --source   Source rollup: a or b
  --dest     Destination rollup: a or b

Example:
  npx ts-node scripts/l2-to-l2-SA-existing-token.ts --source a --dest b
  `);
  process.exit(1);
}
...
function stateFilePath(sourceKey: string, destKey: string): string {
  return path.join(__dirname, `.l2-to-l2-SA-existing-token-state-${sourceKey}-${destKey}`);
}
...
// ---------------------------------------------------------------------------
// First run: deploy token on source, approve bridge for SA, save state
// ---------------------------------------------------------------------------
async function firstRun(
  sourceChain: typeof rollupA | typeof rollupB,
  destChain: typeof rollupA | typeof rollupB,
  sourceKey: string,
  destKey: string,
  srcEthersWallet: ethers.Wallet,
  dstEthersProvider: ethers.JsonRpcProvider,
  saAddress: string
): Promise<State> {
  const tag = `[SA:${sourceChain.name}->${destChain.name}:TOKEN]`;

  // Deploy token on source
  console.log(`\n${tag} Step 1: Deploying test ERC-20 on ${sourceChain.name}...`);
  const tokenFactory = new ethers.ContractFactory(MINTABLE_TOKEN_ABI, MINTABLE_TOKEN_BYTECODE, srcEthersWallet);
  const deployTx = await tokenFactory.deploy("ExistingTokenSA", "ETSA", 18);
  ...
  console.log(`\n${tag} Step 2: Computing predicted CET on ${destChain.name}...`);
  ...
  console.log(`${tag} Predicted CET on ${destChain.name}: ${predictedCET}`);

  // Approve bridge for max from the smart account
  // We do this by minting 0 tokens just to set up, then the SA will approve via UserOp on first bridge.
  // Actually, since the SA calls approve in each bridge UserOp, we don't need a separate approval step.
  // But let's approve from EOA too in case we want to use it later.
  console.log(`\n${tag} Step 3: Done (SA will approve in each bridge UserOp)`);
...
// ---------------------------------------------------------------------------
// Subsequent runs: mint to SA, compose bridge + receive + transfer to EOA
// ---------------------------------------------------------------------------
async function bridgeRun(
  ...
  const srcEthersProvider = srcEthersWallet.provider!;
  ...
  // Step 1: Mint 100 tokens to smart account
  console.log(`\n${tag} Step 1: Minting 100 tokens to smart account...`);
  ...
  console.log(`${tag} SA token balance on source: ${ethers.formatUnits(saTokenBalance, decimals)}`);
  ...
  console.log(`${tag} EOA CET balance on dest before: ${ethers.formatUnits(eoaCetBalanceBefore, decimals)}`);

  // Step 2: Build composed UserOps
  const sessionId = BigInt(Date.now());
  console.log(`\n${tag} Step 2: Building composed transaction...`);
  ...
  // Step 3: Create UserOps + compose + submit
  ...
  // Step 5: Assert balances
  console.log(`\n${tag} Checking final balances...`);

  // Source: SA token balance should have decreased by BRIDGE_AMOUNT
  const saTokenBalanceAfter: bigint = await token.balanceOf(saAddress);
  const srcDecrease = BigInt(saTokenBalance) - BigInt(saTokenBalanceAfter);
  console.log(`${tag} SA token balance on source after: ${ethers.formatUnits(saTokenBalanceAfter, decimals)}`);
  console.log(`${tag} SA source balance decreased by: ${ethers.formatUnits(srcDecrease, decimals)}`);
  if (srcDecrease.toString() !== BRIDGE_AMOUNT.toString()) {
    throw new Error(
      `ASSERT FAILED: SA source balance should have decreased by ${ethers.formatUnits(BRIDGE_AMOUNT, decimals)}, ` +
      `but decreased by ${ethers.formatUnits(srcDecrease, decimals)}`
    );
  }
  console.log(`${tag} ASSERT OK: Source SA token balance decreased by expected amount`);
  ...
  console.log(`${tag} EOA CET balance on dest: ${ethers.formatUnits(eoaCetBalanceAfter, decimals)}`);
  console.log(`${tag} EOA CET increased by: ${ethers.formatUnits(eoaCetIncrease, decimals)}`);

  if (eoaCetIncrease.toString() !== BRIDGE_AMOUNT.toString()) {
    throw new Error(
      `ASSERT FAILED: Expected EOA CET increase of ${ethers.formatUnits(BRIDGE_AMOUNT, decimals)}, ` +
      `got ${ethers.formatUnits(eoaCetIncrease, decimals)}`
    );
  }
  console.log(`${tag} ASSERT OK: EOA received bridged tokens on ${destChain.name}`);

  console.log(`\n${tag} Bridge complete! Tokens are in EOA: ${eoaAddress}`);
  console.log("  Tx hashes:", sendResult.hashes);
...
async function main() {
  ...
  if (!state) {
    console.log(`\n${tag} No state file found — first run: deploying token.`);
    state = await firstRun(sourceChain, destChain, sourceKey, destKey, srcEthersWallet, dstEthersProvider, saAddress);
    console.log(`\n${tag} First run complete. Run again to mint and bridge.`);
  } else {
    console.log(`\n${tag} State loaded — token: ${state.tokenAddress}`);
    await bridgeRun(state, sourceChain, destChain, srcEthersWallet, dstEthersProvider, srcSA, dstSA, saAddress, eoaAddress);
  }
}
